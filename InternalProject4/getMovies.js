...
var url = 'mongodb://localhost:27017/InternalProject4';
		...
		.connect(
				...
				function(err, db) {
					if (err) {
						...
						var collection = db.collection('movieList');
...
app.get("/movies", function(req, res) {
	...
	MongoClient.connect(url, function(err, db) {
		...
		var cursor = db.collection('movieList').find().toArray(
				function(err, docs) {
					res.send(docs);
				});
...
app.get('/getDetails/:id', function(req, res) {
	...
	MongoClient.connect(url, function(err, db) {
		...
		db.collection('movieList').findOne({
